...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        x = self.proj(x)  # (B, 3, 224, 224) -> (B, 768, 14, 14) 224/16=14  B 是批量大小
        x = x.flatten(2).transpose(1, 2)  # (B, 768, 14, 14) -> (B, 196, 768) 转化为序列
        return x
...
class MLP(nn.Module):
    def __init__(self, in_features, hidden_features = None, out_features = None, drop = 0.):
        ...
        self.drop1 = nn.Dropout(drop)  # 随机dropout
...
def vit_patch8_emb256_dep2(model_path=None):
    model = VisionTransformer(
        img_size=32,  # 输入图像大小 32x32
        patch_size=8,  # 每个 Patch 的大小为 8x8
        embed_dim=256,  # 嵌入维度设置为 512
        depth=2,  # Transformer block 的数量设置为 6
        num_heads=8,  # 每个 block 中的注意力头数设置为 8
        mlp_ratio=4,  # MLP 层的比例设置为 4
        qkv_bias=True,  # 使用 QKV 偏置
        num_classes=10  # 输出类别数为 10 (适用于 CIFAR-10 数据集)
    )

    # 如果需要调试可以打印提示
    if model_path is None:
        print(f"预训练权重路径被忽略: {model_path}")
    else:
        print(f"预训练权重路径使用: {model_path}")
        checkpoint = torch.load(model_path, map_location = 'cpu')
        checkpoint = _conv_filter(checkpoint)
        model.load_state_dict(checkpoint, strict = True)
        print(model)
        
    print(f"使用的模型名称: vit_patch8_emb256_dep2")
    return model

def vit_baseline(model_path=None):
    model = VisionTransformer(
        img_size=32, 
        patch_size=16, 
        embed_dim=768, 
    ...
    print(f"使用的模型名称: vit_baseline")
    return model



if __name__ == '__main__':
    model_path = None  # 或者指定预训练权重路径，如 'path/to/weights.pth'
    # 创建模型
    model = vit_baseline(model_path)
